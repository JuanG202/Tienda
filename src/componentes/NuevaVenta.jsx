...
function NuevaVenta() {
    ...
    const cargarDatos = async () => {
      try {
        ...
        if (clienteId !== "seleccionar") {
          const encontrado = clientesRes.data.find((c) => c._id === clienteId);
  ...
  const itemsSeleccionados = productos.filter(
    (p) => (cantidades[p._id] || 0) > 0,
  );
  const total = itemsSeleccionados.reduce(
    (acc, p) => acc + p.precio * (cantidades[p._id] || 0),
    0,
  );
  ...
  const confirmarVenta = async () => {
    ...
    try {
      ...
      const resumenItems = itemsSeleccionados
        .map((p) => `${p.nombre} x${cantidades[p._id]}`)
        .join(", ");
              ...
              <div key={p._id} className="producto-venta-card">
                ...
                <div className="contador">
                  <button
                    ...
                    onClick={() => cambiarCantidad(p._id, -1)}
                    disabled={cant === 0}
                  >
                    −
                  </button>
          ...
          <div className="resumen-venta">
            ...
            <div className="resumen-items">
              {itemsSeleccionados.map((p) => (
                <div key={p._id}>
                  {p.emoji} {p.nombre} ×{cantidades[p._id]} — $
                  {(p.precio * cantidades[p._id]).toLocaleString("es-CO")}
                </div>
              ))}
            </div>
